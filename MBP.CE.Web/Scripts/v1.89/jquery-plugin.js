...
function setValidate() {
    ...
    $.validator.addMethod("daymonthyearfour", function (value) {
        if (value !== "") {
    ...
    $.validator.addMethod("greaterOrEqualThan", function (value, element, params) {
        var elementValue = getEditorValue(element.id);
        var otherValue = getEditorValue(params[0]);

        if (elementValue instanceof Date) {
            return elementValue >= otherValue;
        }

        return isNaN(elementValue) && isNaN(otherValue) || (Number(elementValue) >= Number(otherValue));
    }, "{1}");
